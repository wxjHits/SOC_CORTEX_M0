...
//SPI_FLASH
void SPI_Init(uint8_t SPI_DIV_CLK);      //SPI 时钟分频
uint8_t Is_SPI_Ready(void);                 //返回SPI READY信号
void SPI_SendData(uint8_t TxData);
uint8_t SPI_ReadWriteByte(uint8_t TxData);  //SPI读写信号
...
void SPI_FLASH_Write_Disable(void);	//写失能
